{- EVE Online mining bot version 2020-05-27
   ...
   Modified By AlexNeled
    + cloned from eve-online-mining-bot to eve-online-mining-ICE-bot
    + Used code from eve-online-ratting-anomaly-bot to enhance mining ICE
    + changed for anomaly ice mining instead of asteroid belts 
    + orbit the ice block instead of approach (2,500m)
    + Doing -> set variable for orbit distance
...
-}
{-
   bot-catalog-tags:eve-online,mining,ice,anomaly
   authors-forum-usernames:viir
   Modified By Alex (2020-05-22)
-}
...
defaultBotSettings =
    { runAwayShieldHitpointsThresholdPercent = 90
    , targetingRange = 9500
                                            ...
                                            |> Maybe.withDefault
                                                (DescribeBranch "I see a locked target."
                                                    (case seeUndockingComplete.shipUI.moduleButtonsRows.top |> List.filter (.isActive >> Maybe.withDefault False >> not) |> List.head of
                                                        -- TO-DO: Check previous memory reading too for module activity.
                                                        Nothing ->
                    ...
                    (if shipManeuverIsShipOrbiting readingFromGameClient then
                        ...
                        (actStartingWithRightClickOnOverviewEntry
                            ...
                            [ ( "Click menu entry 'orbit'."
                              ...
                              , ( "Click on menu entry '2,500 m'"
                                , lastContextMenuOrSubmenu
                                     >> Maybe.andThen (menuEntryContainingTextIgnoringCase "2,500 m")
                                    >> Maybe.map (.uiNode >> clickOnUIElement MouseButtonLeft >> List.singleton)
                                )
                            ]  
                        )
    ...
    readingFromGameClient
        |> useContextMenuOnListSurroundingsButton
            ...
            , ( "Click on one of the menu entries."
              , lastContextMenuOrSubmenu
                    >> Maybe.andThen
                        (.entries >> listElementAtWrappedIndex (getEntropyIntFromReadingFromGameClient readingFromGameClient))
                    >> Maybe.map (.uiNode >> clickOnUIElement MouseButtonLeft >> List.singleton)
              )
            , ( "Click menu entry 'Warp to Within'"
              , lastContextMenuOrSubmenu
...
useContextMenuOnListSurroundingsButton : List ( String, ReadingFromGameClient -> Maybe (List VolatileHostInterface.EffectOnWindowStructure) ) -> ReadingFromGameClient -> DecisionPathNode
